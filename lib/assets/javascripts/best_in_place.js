...
BestInPlaceEditor.prototype = {
  ...
  submitForm : function(event) {
    var $form = event.data.editor.element.find('form');

    $form.ajaxSuccess(function(evt, xhr, settings) {
      event.data.editor.loadSuccessCallback(xhr.responseText);
    }).ajaxError(function(evt, request) {
      event.data.editor.loadErrorCallback(xhr, xhr.statusText);
    });
    $form.submit();
    jQuery(this.activator).unbind('click', this.clickHandler);
    jQuery(this.activator).bind('click', {editor: this}, this.clickHandler);
  ...
  /* Generate the data sent in the PUT request */
  ...
  "input" : {
    activateForm : function() {
      var output = $(document.createElement('form'))
                   .addClass('form_in_place lunchbox')
      ...
      var method = $(document.createElement('input'))
                   ...
                   .attr('type', 'hidden')
                   .val('put');
      var csrfProt = $(document.createElement('input'))
                     .attr('type', 'hidden')
                     .attr('name', jQuery('meta[name=csrf-param]').attr('content'))
                     .val(jQuery('meta[name=csrf-token]').attr('content'));
      ...
      this.element.find("input[type='text']").bind('keydown', function(event) {
        return event.keyCode != 13;
      });
      this.element.find("input[type='text']").bind('keypress', function(event) {
        return event.keyCode != 13;
      });
    ...
    getValue : function() {
      return this.sanitizeValue(this.element.find("input[type!='hidden']").val());
    },
    ...
    inputBlurHandler : function(event) {
      if (event.data.editor.okButton) {
        ...
        if (event.data.editor.cancelButton) {
          event.data.editor.blurTimer = setTimeout(function () {
            if (!event.data.editor.userClicked) {
              event.data.editor.submitForm(event);
        ...
        .datepicker({
            onClose: function() {
              that.ajax({
                ...
                "success"    : function(data){ that.loadSuccessCallback(data); },
                "error"      : function(request, error){ that.loadErrorCallback(request, error); }
              });
              that.update();
            }
          })
    ...
    activateForm : function() {
      var output     = $(document.createElement('form'))
          ...
          method     = $(document.createElement('input'))
                       ...
                       .attr('type', 'hidden')
                       .val('put'),
          csrfProt   = $(document.createElement('input'))
                       .attr('type', 'hidden')
                       .attr('name', jQuery('meta[name=csrf-param]').attr('content'))
                       .val(jQuery('meta[name=csrf-token]').attr('content'))
